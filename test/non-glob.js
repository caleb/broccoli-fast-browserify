...
tape.test("non-glob browserify configuration", function(t) {
  t.test("builds the correct bundles in the correct output directory", function(t) {
    ...
    testBroc.builder.build().then(function() {
      ...
      t.notOk(fs.existsSync(path.join(testBroc.tree.outputPath, 'non-glob/non-entry-point.js')), "non-entry point files not in output tree");
      t.notOk(fs.existsSync(path.join(testBroc.tree.outputPath, 'non-glob/non-required.js')), "non-required file not in output tree");
      t.end();
    }).finally(function() {
